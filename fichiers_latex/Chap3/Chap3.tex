...
\begin{figure}[hbtp]
	...
	\matrix [column sep=5mm,row sep=2em] {
		...
		& \node [block_noborder] {Réduction de dimension}; \\
		% row 3
		...
		& \node [block_noborder] {Inférence de la conformité en sortie de machine}; \\
	};% end matrix
...
% Nos travaux nous ont permis d'évaluer différentes méthodes de construction du jeu de données d'apprentissage, vis-à-vis des contraintes industrielles.
...
% Les méthodes par plans d'expériences sont adaptées à la construction d'un jeu de données représentatif initial.
...
\subsection{Réglage automatique du procédé pour la création d'un jeu de données} \label{auto_construction}
...
Dans le cas où la phase de réglage serait automatisée, il deviendrait possible d'explorer la plage de fonctionnement du procédé, tout en ajustant les réglages.  %, à l'aide des méthodes d'optimisation que nous présentons dans la Section \ref{sec:auto_ml}.
...
% \textit{A contrario}, l'annotation hors ligne peut être réalisée en dehors de la ligne de production.
...
\subsubsection{Annotation hors production} \label{parag:offline_annotation}
...
% \paragraph{Expérimentation de l'annotation hors production}\mbox{} \label{parag:offline_annotation}
...
\end{figure}
La Figure \ref{fig:annotation} présente la réalisation de l'annotation hors-production.
...
\begin{itemize}
	\item la durée de trois heures avec quatre experts pour évaluer deux cents pièces, soit soixante pièces par heure,  % Cette cadence est équivalente au temps de cycle de production d'une pièce par minute.
...
% la ludification du processus d'annotation peut aussi être un facteur important.
...
Nous avons développé une solution logicielle afin de satisfaire cette nécessité.
...
Il est alors intéressant de suggérer à l'expert humain une annotation pour la pièce.
...
Pour obtenir un détecteur de nouveauté, il reste à définir un seuil à partir duquel la pièce est considérée comme nouvelle.
...
Nous n'avons pas pu évaluer notre proposition dans le cadre de ce travail, car cela aurait nécessité l'enregistrement automatique de la valeur des réglages de la machine.
...
\miniconclusion{Synthèse : Transmission de l'expertise humaine par l'annotation d'un jeu de données}{
	...
	Les plans d'expériences permettent d'explorer rapidement la réponse d'un système, puis il est possible d'enrichir le jeu de données au fur et à mesure de la créations de nouveaux individus par une annotation production.
...
C'est à dire que nous cherchons à classer automatiquement dans des groupes les individus qui ont des propriétés similaires.  % , mesurées sur des observations.
...
Pour un classifieur, $y$ est souvent un vecteur de la probabilité d'appartenance de l'individu à chacune des classes.
...
Aussi, en comparant les entropies d'une distribution source $y$ et d'une approximation de $\hat{y}$, on mesure la quantité d'information perdue par la méthode d'approximation $F$.  % F est aussi appelée "méthode d'encodage".
...
%Les annotations sont propagées de manière itérative depuis les individus annotés, vers les individus non annotés.
%À chaque itération, les individus nouvellement annotés sont utilisés pour continuer à propager les annotations.
...
%De plus, l'hypothèse des variétés est utilisée pour réaliser la propagation des annotations dans un espace latent, à partir des annotations des individus annotés, vers les individus non-annotés, qui sont voisins.
...
%A partir des images brutes non modifiées, nous obtenons alors un jeu de données initiales avec pour chaque individu un vecteur de dimensions $1920 \times 7 \times 7 = 94 080$.
...
%Le coût d'utilisation de ces deux méthodes est élevé : $\bigO(N^3)$, avec $N$ le nombre d'individus en présence.
%
%Dès 2002, \citeauthor{zhu_learning_2002} \cite{zhu_learning_2002} proposent de construire le graphe qui relie l'ensemble des individus par le chemin le plus court.
%L'apprentissage est réalisé de manière itérative en propageant les annotations à partir des individus annotés, vers les individus non annotés.
%La convergence de la méthode est obtenue lorsque tous les individus sont annotés.
%Soit $Y_{L}=\left\{y_{1} \ldots y_{l}\right\}$ les annotations des $l$ individus annotés et $Y_{U}=\left\{y_{l+1} \ldots y_{l+U}\right\}$ les annotations des $u$ individus non annotés.
%Enfin, $X=\left\{x_{1} \ldots x_{l+u}\right\}$ les valeurs de l'ensemble des individus, annotés et non annotés.
%Le problème cherche à trouver les valeurs de $Y_{U}$ pour tous les $X$ et les $Y_{L}$.
%Un graphe pondéré $w_{ij}$ entre les individus dans l'espace latent est construit. Il est pondéré en fonction de la distance Euclidienne entre chaque individu, les individus proches ayant un poids plus élevé.
%$w$ est aussi appelé la matrice d'adjacence.
%Enfin, le graphe de transition entre les individus est construit : $T_{i j}=P(j \ i)=\frac{w_{i j}}{\sum_{k=1}^{l+u} w_{k j}}$.
%L'algorithme consiste alors à appliquer $T$ à $Y$ ($Y = Y \cdot T$) jusqu'à obtenir le critère de convergence.
%À chaque itération de l'algorithme, la pondération des individus annotés est augmentée par un facteur $\alpha$ afin de rester supérieure aux probabilités des individus non annotés.
...
%Cette matrice de transition modélise les distances entre les individus.
%
...
%Sur notre problème qui peut comporter mille individus, le gain est déjà d'un facteur mille.
...
\begin{figure}[hbtp]
	...
	\caption{Principe de l'apprentissage d'un réseau de triplets.}
	\textit{Figure inspirée de \cite{schroff_facenet_2015}.}
...
\miniconclusion{Synthèse : Construction d'une métrique de la qualité d'une pièce par apprentissage}{
	...
	Notre contexte industriel limite le nombre d'individus dans nos jeux de données d'apprentissage, c'est pourquoi nous nous sommes intéressés aux méthodes d'apprentissage non-supervisés et semi-supervisés. 
